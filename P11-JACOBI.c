...
for(j=0;j<5;j++){
	...
	if(j==0){
		...
		ERR1=fabs((x_ite_1-X1D)/(x_ite_1))*100;
        X2D = x_ite_2;
        x_ite_2 = ((-A[1][0]*X1D)-(A[1][2]*x_ite_3)+b[1])/A[1][1];
       
	    ERR2=fabs((x_ite_2-X2D)/(x_ite_2))*100;
        X3D = x_ite_3;
        x_ite_3 = ((-A[2][0]*X1D)-(A[2][1]*X2D)+b[2])/A[2][2];
        
		ERR3=fabs((x_ite_3-X3D)/(x_ite_3))*100;
        	
			printf("x_1 en la iteracion %d es igual a %f, Con un error del %f %%\n",j+1,x_ite_1,ERR1);
        	
			printf("x_2 en la iteracion %d es igual a %f, Con un error del %f %%\n",j+1,x_ite_2,ERR2);
        	
			printf("x_3 en la iteracion %d es igual a %f, Con un error del %f %%\n",j+1,x_ite_3,ERR3);
        }
    }
}
